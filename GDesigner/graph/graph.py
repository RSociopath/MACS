    ...
    async def arun(self, input: Dict[str,str], 
       ...
       # output_dir = f"/home/teachhu/wyh/{dir}"
        ...
        for round in range(num_rounds):
            ...
            in_degree = {node_id: len(node.spatial_predecessors) for node_id, node in self.nodes.items()}
            in_degrees = {node_id: len(node.spatial_predecessors) for node_id, node in self.nodes.items()}
            zero_in_degree_queue = [node_id for node_id, deg in in_degree.items() if deg == 0]
            zero_in_degree_queues = [node_id for node_id, deg in in_degree.items() if deg == 0]
            count=0
            ...
            while zero_in_degree_queue:
                ...
                if len(self.nodes[current_node_id].spatial_predecessors):
                    # with open(output_file, 'a') as f:
                    #     f.write(f"################current_node:{self.nodes[current_node_id].id}\n")
                    count+=1
                tries = 0
                while tries < max_tries:
                    try:
                ...
                for successor in self.nodes[current_node_id].spatial_successors:
                    ...
                    if in_degree[successor.id] == 0:
                        zero_in_degree_queue.append(successor.id)
                        # with open(output_file, 'a') as f:
                        #     f.write(f"################current_node_successor:{successor.id}\n")
            self.update_memory()
        ...
        if len(final_answers) == 0:
            final_answers.append("No answer of the decision node")
        return final_answers, log_probs,similarities
